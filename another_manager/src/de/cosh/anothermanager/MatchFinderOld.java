 ...
 */
public class MatchFinderOld {

    private int MAX_SIZE_X, MAX_SIZE_Y;
    private CellOld[][] board;

    public boolean findMatches(CellOld[][] board, boolean mark) {
    ...
    private int howManyMatchesToTheTop(int x, int y) {
        GemOld.GemType currentType = board[x][y].getOccupant().getGemType();
        if (currentType == GemOld.GemType.TYPE_NONE)
            return 0;
        int searchPos = y;
        int count = 1;
    ...
    public void init(CellOld[][] board, int max_size_x, int max_size_y) {
